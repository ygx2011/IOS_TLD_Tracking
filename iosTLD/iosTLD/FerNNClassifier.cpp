...
void FerNNClassifier::read(/*const FileNode& file*/){
  ///Classifier Parameters
    valid = 0.5;//(float)file["valid"];
    ncc_thesame = 0.95;//(float)file["ncc_thesame"];
    nstructs = 10;//(int)file["num_trees"];
    structSize = 13;//(int)file["num_features"];
    thr_fern = 0.6;//(float)file["thr_fern"];
    thr_nn = 0.65;//(float)file["thr_nn"];
    thr_nn_valid = 0.7;//(float)file["thr_nn_valid"];
}
